...
NUM_EPOCHS = 15
SEED = 56297
z_dim = 200  # original latent size
input_shape = (32, 32, 32, 1)
LEARNING_RATE = 0.001

cur_path = "/home/x_leomo/saxsdiff/users/Diff_SAXS_reconstruction/"
folder_of_well_trained_model = os.path.join(cur_path, 'model_1')
folder_to_save_model = os.path.join(cur_path, 'model_2')
...
def build_decoder(z_dim=z_dim):
    ...
    outputs = tf.keras.layers.Conv3D(1, 3, padding='same')(x)  # logits
...
# -------------------
# Load previously trained weights if available (with skip-mismatch trick)
# -------------------
weights_path = os.path.join(folder_of_well_trained_model, "best_model_weights.h5")
if os.path.exists(weights_path):
    try:
        autoencoder.load_weights(weights_path, by_name=True, skip_mismatch=True)
        print(f"Loaded compatible weights from {weights_path} (skip mismatches)")
    except Exception as e:
        print(f"Could not fully load weights from {weights_path}: {e}")
# -------------------
# Compile model
# -------------------
autoencoder.compile(
